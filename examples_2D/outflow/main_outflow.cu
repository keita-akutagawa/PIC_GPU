...
std::string directoryname = "results_outflow";
std::string filenameWithoutStep = "outflow";
...
const float tElectron = (B0 * B0 / 2.0f / mu0) / (numberDensityIon + numberDensityElectron * tRatio);
...
const float bulkVxElectron = c * debyeLength / sheatThickness * sqrt(2.0f / (1.0f + 1.0f/tRatio)) * sin(-theta);
const float bulkVyElectron = 0.0f;
const float bulkVzElectron = c * debyeLength / sheatThickness * sqrt(2.0f / (1.0f + 1.0f/tRatio)) * cos(theta);
...
)
{
    ...
    if (i < device_nx && j < device_ny) {
        ...
        B[j + device_ny * i].bX = device_B0 * tanh((j * device_dy - yCenter) / device_sheatThickness) * cos(device_theta);
        B[j + device_ny * i].bY = 0.0f; 
        B[j + device_ny * i].bZ = device_B0 * tanh((j * device_dy - yCenter) / device_sheatThickness) * sin(device_theta);
    }
}

void PIC2D::initialize()
{
    cudaMemcpyToSymbol(device_theta, &theta, sizeof(float));
