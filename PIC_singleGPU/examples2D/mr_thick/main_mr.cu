...
std::string directoryname = "/cfca-work/akutagawakt/PIC/results_mr_thick_small";
std::string filenameWithoutStep = "mr";
std::ofstream logfile("/cfca-work/akutagawakt/PIC/results_mr_thick_small_2/log_mr.txt");
...
const int nx = round(1000.0f * ionInertialLength);
const float dx = 1.0f;
...
const int ny = round(500.0f * ionInertialLength);
const float dy = 1.0f;
...
//追加
const float sheatThickness = 20.0f * ionInertialLength;
...
//追加
__constant__ unsigned long long device_harrisNumIon;
__constant__ unsigned long long device_backgroundNumIon;
__constant__ unsigned long long device_totalNumIon;
__constant__ unsigned long long device_harrisNumElectron;
__constant__ unsigned long long device_backgroundNumElectron;
...
)
{
    ...
    if (i < device_nx && j < device_ny) {
        float xCenter = 0.5f * (device_xmax - device_xmin) + device_xmin;
        float yCenter = 0.5f * (device_ymax - device_ymin) + device_ymin;
        float x = i * device_dx, y = j * device_dy;
        float VA = device_B0 / sqrt(device_mu0 * (device_numberDensityIon * device_mIon + device_numberDensityElectron * device_mElectron));
        ...
        B[j + device_ny * i].bX = device_B0 * tanh((j * device_dy - yCenter) / device_sheatThickness)
                                ...
                                * exp(-(pow((i * device_dx - device_xPointPosition) / 20.0, 2) + pow((j * device_dy - yCenter), 2))
                                / pow(2.0f * device_sheatThickness, 2));
        B[j + device_ny * i].bY = device_B0 * device_triggerRatio * (i * device_dx - device_xPointPosition) / 20.0 / device_sheatThickness
                                * exp(-(pow((i * device_dx - device_xPointPosition) / 20.0, 2) + pow((j * device_dy - yCenter), 2))
                                / pow(2.0f * device_sheatThickness, 2)); 
