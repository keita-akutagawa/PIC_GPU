...
)
{
    ...
    dim3 threadsPerBlockForHeavyIon(256);
    dim3 blocksPerGridForHeavyIon((totalNumHeavyIon + threadsPerBlockForHeavyIon.x - 1) / threadsPerBlockForHeavyIon.x);

    periodicBoundaryParticleX_kernel<<<blocksPerGridForHeavyIon, threadsPerBlockForHeavyIon>>>(
        thrust::raw_pointer_cast(particlesHeavyIon.data()), totalNumHeavyIon
    );
...
)
{
    ...
    conductingWallBoundaryParticleY_kernel<<<blocksPerGridForHeavyIon, threadsPerBlockForHeavyIon>>>(
        thrust::raw_pointer_cast(particlesHeavyIon.data()), totalNumHeavyIon
    );
